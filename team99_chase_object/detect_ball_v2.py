...
class MinimalCoordinatePublisher(Node):
    ...
    def __init__(self):
        ...
        self.coor_publisher_ = self.create_publisher(Int64, '/lab3/pixel_pos', 5)
...
class MinimalVideoSubscriber(Node):
    ...
    def process_img(self, img):
        ...
        # self.blur_frame1 = cv2.GaussianBlur(self.gray_frame, (15, 15), 1.0)
        # self.blur_frame2 = cv2.medianBlur(self.blur_frame1, 7)

        self.HSVframe = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

        # Orange
        # lower = np.array([136,49,95])
        # upper = np.array([179,255,255])

        # Blue
        lower = np.array([72,76,75])
        upper = np.array([123,220,255])

        # Yellow
        # lower = np.array([23, 33, 151])
        # upper = np.array([42, 158, 255])


        self.mask = cv2.inRange(self.HSVframe, lower, upper)
        self.blur_frame2 = cv2.medianBlur(self.mask, 25)
    ...
    def get_circle_ls(self, s_frame):
        ...
        self.circle_ls = cv2.HoughCircles(s_frame, cv2.HOUGH_GRADIENT, 2.2, 150, param1=100, param2=40, minRadius=50,
                                          maxRadius=0)
        # self.circle_ls = cv2.HoughCircles(s_frame, cv2.HOUGH_GRADIENT, 1.5, 100, param1=100, param2=50, minRadius=100, maxRadius=400)
        # self.circle_ls = cv2.HoughCircles(s_frame, cv2.HOUGH_GRADIENT, 1.5, 100, param1=100, param2=50, minRadius=100, maxRadius=0)
            ...
            if circle_ls is not None:
                ...
                for i in circle_ls[0, :]:
                    ...
                    if prev_circle is not None:
                        if calc_dist(main_circle[0], main_circle[1], prev_circle[0], prev_circle[1]) <= calc_dist(i[0],
                                                                                                                  i[1],
                                                                                                                  prev_circle[
                                                                                                                      0],
                                                                                                                  prev_circle[
                    ...
                    cv2.putText(main_frame, txt_mark, org=(main_circle[0], main_circle[1]),
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(102, 255, 51), thickness=2,
                                lineType=cv2.LINE_AA)
                    print('testest')
                    print("Ball Position: ", main_circle[0], main_circle[1])
                    coor_publisher.send_ball_coordinates(int(main_circle[0]))
                    prev_circle = main_circle
            else:
                coor_publisher.send_ball_coordinates(10000)

            video_subscriber.show_image(video_subscriber.get_image())
            # video_subscriber.show_image(edit_frame)
